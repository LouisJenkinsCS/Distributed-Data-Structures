...
var padding4 : 7 * int;
var global_elemsProcessed : atomic int;
...
class TreeNode {
  ...
  proc genChildren(q : Queue(TreeNode)): int {
    select distrib {
...
proc ratomic_int64.max(other: int) {
  on this {
  	var curMax = this.read();
  	while curMax < other && !this.compareExchange(curMax, other) do
    	curMax = this.read();
...
proc main() {
  var t_create: Timer;
  var root: TreeNode;
  var queue = new DistributedWorkQueue(TreeNode);
  ...
  coforall loc in Locales do on loc {
    var localQueue = queue.localQueue;
    coforall tid in 0 .. #here.maxTaskPar {
      var (hasElem, elem) : (bool, TreeNode);
      var maxDepth : int;
      while global_count.read() != 0 {
        (hasElem, elem) = localQueue.dequeue();
        if !hasElem {
          chpl_task_yield();
          continue;
        }

        global_count.add(elem.genChildren(localQueue));
        global_elemsProcessed.add(1);
        global_count.sub(1);

        if elem.depth > maxDepth {
          global_maxDepth.max(elem.depth);
          maxDepth = global_maxDepth.read();
        }
